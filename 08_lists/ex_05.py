...
try:
    ...
    for line in fhand:
        ...
        if (len(words) == 0 or len(words) < 3 or words[0] != 'From'):
            continue
        starts_with_from.append(words[1])

    for email in starts_with_from[:3]:
        print(email)

    print('\n')
    print('[...some output removed...]')
    print('\n')

    for email in starts_with_from[3:7]:
        print(email)

    print('\n')
    print('There were ' + str(len(starts_with_from)) +
          ' lines in the file with From as the first word')
except:
    print('Failed to open the file named: ', filename)
